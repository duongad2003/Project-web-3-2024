...
if (isset($_GET['cart_id'])) {
    ...
    $amount = $add['product_amount'];
    if($amount <= 0){
        echo '<script type="text/javascript">
                   window.onload = function () { alert("Sản phẩm này đã hết"); window.location.href = "shop.php"; } 
            </script>';
    }else{
    $redirect_to_cart = isset($_GET['redirect_to_cart']) ? $_GET['redirect_to_cart'] : 0;
    ...
    if (mysqli_num_rows($se_cart) > 0) {
        ...
        mysqli_query($con, "INSERT INTO `btl_cart` (`user_id`, product_id, `name`, `price`, `image`, `quantity`) VALUES ('$id', '$pr_id', '$name', '$price', '$image', '1')") or die('query failed');
        echo '<script type="text/javascript">
                   window.onload = function () { alert("Đã thêm sản phẩm vào giỏ hàng"); window.location.href = "shop.php?"; } 
            </script>';
    }
                    ...
                    <div class="row g-4">
                                ...
                                <div class="row g-4 ">
                                    ...
                                    <div class="col-lg-12">
                                        <div class="position-relative">
                                            <img src="img/ruouvangtrang.jpg" class="img-fluid w-100 rounded" alt="">
                                            <div class="position-absolute" style="top: 50%; right: 10px; transform: translateY(100%);">
                                                <h3 class="text-secondary fw-bold">Alcohol <br> Huduoai <br> Special</h3>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-lg-9 product-list">
                            <div class="row g-3 justify-content-center">
                                ...
                                if ($search) {
                                    $searchQuery = "AND product_name LIKE '%$key%' OR p.catalog_name LIKE '%$key%'";
                                }



                                $searchQueryPart = isset($_SESSION['searchQueryPart']) ? $_SESSION['searchQueryPart'] : '';
                                ...
                                $query = "SELECT p.*, c.catalog_name FROM btl_product p INNER JOIN btl_catalog c ON p.catalog_id = c.catalog_id WHERE 1 $catalogQueryPart $priceQueryPart $searchQuery $srquery LIMIT $start_from, $num_per_page";
                                $select = mysqli_query($con, $query);

                                while ($se = mysqli_fetch_array($select)) {
                                    $formatted_price = number_format($se['product_price'], 0, '.', ',') . ' VNĐ';
                                    $formatted_newprice = number_format($se['product_new_price'], 0, '.', ',') . ' VNĐ'; // assuming there's a 'new_price' field
                                        ...
                                        <div class="border rounded position-relative product-item">
                                            ...
                                            <div class="product-image">
                                                ...
                                                <div class="product-action">
                                                    ...
                                                    <a href="shop.php?cart_id=<?php echo $se['product_id']; ?>&redirect_to_cart=1"><i class="fa-light fa-cart-shopping"></i></a>


                                                    <a href="shop.php?cart_id=<?php echo $se['product_id']; ?>"><i class="fa-light fa-plus"></i></a>
                                                    <a class="fw-bold" href="product-detail.php?id=<?php echo $se['product_id']; ?>"><i class="fa-light fa-magnifying-glass"></i></i></a>

                                                </div>
                                            </div>
                                            <div class="product-title">
                                                <a class="fw-bold" href="product-detail.php?id=<?php echo $se['product_id']; ?>"><?php echo $se['product_name'] ?></a>
                                                <div class="ratting">
                                                    <p>Số Lượng Còn: <?php echo $se['product_amount'] ?></p>
                                                    <?php if ($se['product_new_price'] != 0) { ?>
                                                        <p style="text-decoration: line-through;"><?php echo $formatted_price; ?></p>
                                                        <h3><?php echo $formatted_newprice; ?></h3>
                                                    <?php } else { ?>
                                ...
                                <div class="col-12">
                                    <div class="pagination d-flex justify-content-center mt-5">
                                        <?php
                                        $sql = "SELECT * FROM btl_product p WHERE 1 $catalogQueryPart $priceQueryPart $searchQuery";
                                        $rs_result = mysqli_query($con, $sql);
                                        $total_records = mysqli_num_rows($rs_result);
                                        $total_page = ceil($total_records / $num_per_page);

                                        if ($page > 1) {
                                            echo "<a href='shop.php?page=" . ($page - 1) . "&sr=" . $sr . "&key=" . $key . "' class='rounded'>&laquo;</a>";
                                        }

                                        for ($i = 1; $i <= $total_page; $i++) {
                                            if ($i == $page) {
                                                echo "<a href='shop.php?page=" . $i . "&sr=" . $sr . "&key=" . $key . "' class='active rounded'>" . $i . "</a>";
                                            } else {
                                                echo "<a href='shop.php?page=" . $i . "&sr=" . $sr . "&key=" . $key . "' class='rounded'>" . $i . "</a>";
                                            }
                                        }

                                        if ($page < $total_page) {
                                            echo "<a href='shop.php?page=" . ($page + 1) . "&sr=" . $sr . "&key=" . $key . "' class='rounded'>&raquo;</a>";
                                        }
                                        ?>
                                    </div>
                                </div>
